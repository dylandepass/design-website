 ...
 * @param {Element} main The container element
 * @preserve
 */function a(e){e.querySelectorAll("div.section > div > div").forEach((e=>n(e)))}
/**
 ...
 * @preserve
 */function d(e,t){const o=t.map((e=>e.toLowerCase()));e.querySelectorAll("h1, h2, h3, h4, h5, h6").forEach((e=>{const t=e.tagName.toLowerCase();if(-1===o.indexOf(t)){let n=parseInt(t.charAt(1),10)-1;for(;-1===o.indexOf(`h${n}`)&&n>0;)n-=1;if(0===n)for(;-1===o.indexOf(`h${n}`)&&n<7;)n+=1;7!==n&&(e.outerHTML=`<h${n}>${e.textContent}</h${n}>`)}}))}
/**
 * Adds the favicon.
 * @param {string} href The favicon URL
 * @preserve
 */function l(e){const t=document.createElement("link");t.rel="icon",t.type="image/svg+xml",t.href=e;const o=document.querySelector('head link[rel="icon"]');o?o.parentElement.replaceChild(t,o):document.getElementsByTagName("head")[0].appendChild(t)}
/**
 ...
 * @param {Element} main The container element
 * @preserve
 */function w(e){const t=[...e.querySelectorAll(":scope > div.section")];for(let e=0;e<t.length;e+=1){const o=t[e];if("loaded"!==o.getAttribute("data-section-status")){if(o.querySelector('.block[data-block-status="initialized"], .block[data-block-status="loading"]')){o.setAttribute("data-section-status","loading");break}o.setAttribute("data-section-status","loaded")}}}
/**
 * Loads JS and CSS for a block.
 * @param {Element} block The block element
 * @preserve
 */async function f(e,t=!1){if("loading"!==e.getAttribute("data-block-status")&&"loaded"!==e.getAttribute("data-block-status")){e.setAttribute("data-block-status","loading");const o=e.getAttribute("data-block-name");try{const n=new Promise((e=>{m(`/blocks/${o}/${o}.css`,e)})),a=new Promise((n=>{(async()=>{try{const n=await import(`${window.hlx.codeBasePath}/blocks/${o}/${o}.js`);n.default&&await n.default(e,o,document,t)}catch(e){console.log(`failed to load module for ${o}`,e)}n()})()}));await Promise.all([n,a])}catch(e){console.log(`failed to load block ${o}`,e)}e.setAttribute("data-block-status","loaded")}}
/**
 * Loads JS and CSS for all blocks in a container element.
 * @param {Element} main The container element
 * @preserve
 */async function p(e){w(e);const t=[...e.querySelectorAll("div.block")];for(let o=0;o<t.length;o+=1)await f(t[o]),w(e)}
/**
 * Builds a block DOM Element from a two dimensional array
 * @param {string} blockName name of the block
 * @param {any} content two dimensional array or string or object of content
 * @preserve
 */function y(e,t){const o=Array.isArray(t)?t:[[t]],n=document.createElement("div");return n.classList.add(e),o.forEach((e=>{const t=document.createElement("div");e.forEach((e=>{const o=document.createElement("div");(e.elems?e.elems:[e]).forEach((e=>{e&&("string"==typeof e?o.innerHTML+=e:o.appendChild(e))})),t.appendChild(o)})),n.appendChild(t)})),n}
/**
 * Loads the header block.
 * @param {Element} header The header element
 * @preserve
 */async function g(e,t){const o=y("header","");e.append(o),n(o),await f(o),u(o,t)}
/**
 * Loads the footer block.
 * @param {Element} footer The footer element
 * @preserve
 */async function b(e,t){const o=y("footer","");e.append(o),n(o),await f(o),u(o,t)}
/**
 * load LCP block and/or wait for LCP in default content.
 * @preserve
 */async function A(e){const t=e,o=document.querySelector(".block");o&&t.includes(o.getAttribute("data-block-name"))&&await f(o,!0),document.querySelector("body").classList.add("appear");const n=document.querySelector("main img");await new Promise((e=>{n&&!n.complete?(n.setAttribute("loading","eager"),n.addEventListener("load",(()=>e())),n.addEventListener("error",(()=>e()))):e()}))}
/**
 ...
 * @preserve
 */function k(e,t,o={}){try{if(window.hlx=window.hlx||{},!window.hlx.rum){const e="on"===new URLSearchParams(window.location.search).get("rum")?1:100,t=`${(e=>e.split("").reduce(((e,t)=>(e<<5)-e+t.charCodeAt(0)|0),0))(window.location.href)}-${(new Date).getTime()}-${Math.random().toString(16).substr(2,14)}`,o=Math.random(),n=o*e<1;window.hlx.rum={weight:e,id:t,random:o,isSelected:n}}const{random:n,weight:a,id:i}=window.hlx.rum;if(n&&n*a<1){const n=()=>{const n=JSON.stringify({weight:a,id:i,referer:window.location.href,generation:t,checkpoint:e,...o}),c=`https://rum.hlx.page/.rum/${a}`;navigator.sendBeacon(c,n)};if(n(),"cwv"===e){const e=document.createElement("script");e.src="https://rum.hlx.page/.rum/web-vitals/dist/web-vitals.iife.js",e.onload=()=>{const e=e=>{o.cwv={},o.cwv[e.name]=e.value,n()};window.webVitals.getCLS(e),window.webVitals.getFID(e),window.webVitals.getLCP(e)},document.head.appendChild(e)}}}catch(e){}}function S(e){window.name.includes("performance")&&console.log(`${new Date-performance.timing.navigationStart}:${e}`)}function $(){try{new PerformanceObserver((e=>{const t=e.getEntries();S(JSON.stringify(t)),console.log(t[0].element)})).observe({type:"largest-contentful-paint",buffered:!0});new PerformanceObserver((e=>{const t=e.getEntries();S(JSON.stringify(t)),console.log(t[0].sources[0].node)})).observe({type:"layout-shift",buffered:!0});new PerformanceObserver((e=>{e.getEntries().forEach((e=>{S(`resource loaded: ${e.name} - [${Math.round(e.startTime+e.duration)}]`)}))})).observe({type:"resource",buffered:!0})}catch(e){}}
/**
 * Initializes helix
 * @preserve
 */function L(){window.hlx=window.hlx||{},window.hlx.lighthouse="on"===new URLSearchParams(window.location.search).get("lighthouse"),window.hlx.codeBasePath="";const e=document.querySelector('script[src$="/scripts/scripts.js"]');if(e)try{window.hlx.codeBasePath=new URL(e.src).href.replace("/scripts/scripts.js","")}catch(e){console.log(e)}}
/**
 * Adds one or more URLs to the dependencies for publishing.
 * @param {string|string[]} url The URL(s) to add as dependencies
 * @preserve
 */function v(e){const t=Array.isArray(e)?e:[e];window.hlx=window.hlx||{},window.hlx.dependencies&&Array.isArray(window.hlx.dependencies)?window.hlx.dependencies.concat(t):window.hlx.dependencies=t}class x{constructor(e){this.config=e,L(),this.config.rumEnabled&&(this.sampleRUM("top"),window.addEventListener("load",(()=>k("load"))),document.addEventListener("click",(()=>k("click")))),window.name.includes("performance")&&$()}static init(e){return new x(e)}withLoadEager(e){return this.loadEagerHook=e,this}withLoadLazy(e){return this.loadLazyHook=e,this}withLoadDelayed(e){return this.loadDelayed=e,this}withBuildAutoBlocks(e){return this.buildAutoBlocks=e,this}withLoadHeader(e){return this.loadHeader=e,this}withLoadFooter(e){return this.loadFooter=e,this}withDecorateSections(e){return this.decorateSections=e,this}withDecorateBlocks(e){return this.decorateBlocks=e,this}withDecorateBlock(e){return this.decorateBlock=e,this}async decorate(){await this.loadEager(document),await this.loadLazy(document),this.loadDelayed(document)}
/**
   * Decorates all blocks in a container element.
   * @param {Element} main The container element
   * @preserve
   */decorateBlocks(e){e.querySelectorAll("div.section > div > div").forEach((e=>this.decorateBlock(e)))}decorateMain(e){s(e),o(e),u(e,this.config.productionDomains),this.buildAutoBlocks(e),this.decorateSections(e),this.decorateBlocks(e)}
/**
   * log RUM if part of the sample.
   * @param {string} checkpoint identifies the checkpoint in funnel
   * @param {Object} data additional data for RUM sample
   * @preserve
   */sampleRUM(e,t={}){k(e,this.config.rumGeneration,t)}async loadEager(e){const t=e.querySelector("main");t&&(this.decorateMain(t),await this.waitForLCP(this.config.lcpBlocks)),this.loadEagerHook&&await this.loadEagerHook(e)}async loadLazy(e){const t=e.querySelector("main");await p(t),this.loadHeader(e.querySelector("header")),this.loadFooter(e.querySelector("footer")),m(`${window.hlx.codeBasePath}/styles/lazy-styles.css`),l(`${window.hlx.codeBasePath}/icon.svg`),this.loadLazyHook&&this.loadLazyHook(e)}loadDelayed(){}buildAutoBlocks(e){}async loadHeader(e){g(e,this.config.productionDomains)}async loadFooter(e){b(e,this.config.productionDomains)}
/**
   * Decorates all sections in a container element.
   * @param {Element} main The container element
   * @preserve
   */decorateSections(e){r(e)}
/**
   * Decorates a block.
   * @param {Element} block The block element
   * @preserve
...
/**
   * load LCP block and/or wait for LCP in default content.
   * @preserve
   */waitForLCP(e){return A(e)}}export{x as HelixApp,l as addFavIcon,v as addPublishDependencies,y as buildBlock,e as createOptimizedPicture,n as decorateBlock,a as decorateBlocks,s as decoratePictures,r as decorateSections,E as getMetadata,t as getOptimizedImagePath,L as initHlx,f as loadBlock,p as loadBlocks,m as loadCSS,b as loadFooter,g as loadHeader,h as loadScript,u as makeLinksRelative,d as normalizeHeadings,c as readBlockConfig,$ as registerPerformanceLogger,o as removeStylingFromImages,k as sampleRUM,S as stamp,i as toClassName,w as updateSectionsStatus,A as waitForLCP};
